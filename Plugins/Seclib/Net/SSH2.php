 ...
 *
 *    $ssh = new \Seclib\Net\SSH2('www.domain.tld');
...
namespace Seclib\Net;

use Seclib\Crypt\Base;
use Seclib\Crypt\Blowfish;
use Seclib\Crypt\Hash;
use Seclib\Crypt\Random;
use Seclib\Crypt\RC4;
use Seclib\Crypt\Rijndael;
use Seclib\Crypt\RSA;
use Seclib\Crypt\TripleDES;
use Seclib\Crypt\Twofish;
use Seclib\Math\BigInteger; // Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use Seclib\System\SSH\Agent;
...
class SSH2
{
     ...
     *     the other side.
     *
     * @see \Seclib\Net\SSH2::_send_channel_packet()
     * @see \Seclib\Net\SSH2::_get_channel_packet()
     * @access private
    */
    ...
    */
     ...
     *
     * @param string $algorithm Name of the encryption algorithm
     * @return mixed Instance of \Seclib\Crypt\Base or null for unknown
     * @access private
     */
     ...
     * Login
     *
     * The $password parameter can be a plaintext password, a \Seclib\Crypt\RSA object or an array
     *
    ...
    function _keyboard_interactive_process()
    {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                ...
                */
                // maybe Seclib should force close the connection after x request / responses?  unless something like that is done
     ...
     *
     * @param string $username
     * @param \Seclib\System\SSH\Agent $agent
     * @return bool
     * @access private
     */
     ...
     *
     * @param string $username
     * @param \Seclib\Crypt\RSA $password
     * @return bool
     * @access private
     * @internal It might be worthwhile, at some point, to protect against {@link http://tools.ietf.org/html/rfc4251#section-9.3.9 traffic analysis}
     *           by sending dummy SSH_MSG_IGNORE messages.
     ...
     * Execute Command
     *
     * If $callback is set to false then \Seclib\Net\SSH2::_get_channel_packet(self::CHANNEL_EXEC) will need to be called manually.
    ...
    function exec($command, $callback = null)
    {
        ...
        // honestly, if you're transferring more than 2GB, you probably shouldn't be using Seclib, anyway.
        ...
        // down.  the one place where it might be desirable is if you're doing something like \Seclib\Net\SSH2::exec('ping localhost &').
     ...
     *
     * \Seclib\Net\SSH2 doesn't properly close most channels.  For exec() channels are normally closed by the server
    ...
    /**
     * Returns a list of KEX algorithms that Seclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of host key algorithms that Seclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of symmetric key algorithms that Seclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of MAC algorithms that Seclib supports
     *
     * @return array
     * @access public
     */
    ...
    /**
     * Returns a list of compression algorithms that Seclib supports
     *
     * @return array
     * @access public
     */
